...
namespace Kivicms\Fias\Elastic\Tests\IndexMapper;

use DateTimeImmutable;
use Kivicms\Fias\Elastic\IndexMapper\ApartmentsIndexMapper;
use Kivicms\Fias\Elastic\QueryBuilder\QueryBuilder;
use Kivicms\Fias\Elastic\Tests\BaseCase;
use stdClass;

/**
 * Тест для описания индекса сущности 'Сведения по помещениям'.
 *
 * @internal
 */
class ApartmentsIndexMapperTest extends BaseCase
{
    public function testGetName(): void
    {
        $mapper = new ApartmentsIndexMapper();

        $this->assertSame('apartments', $mapper->getName());
    }
    ...
    public function testGetMappingProperties(): void
    {
        ...
        $this->assertArrayHasKey('aparttype', $map);
    ...
    public function testExtractDataFromEntity(): void
    {
        ...
        $entity->aparttype = $this->createFakeData()->numberBetween(1, 100000);
        ...
        $this->assertArrayHasKey('aparttype', $dataForElastic);
        $this->assertSame($entity->aparttype, $dataForElastic['aparttype'], 'Test aparttype field conversion.');
